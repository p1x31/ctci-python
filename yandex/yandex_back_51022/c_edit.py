...
for _ in range(Q):
    ...
    for i in range(N):
        ...
        # Если знак сравнения >
        if sign == '>':
            # Если значение в колонке больше заданного значения
            if x > value:
                # Добавляем индекс строки в множество
                s.add(i)
        # Если знак сравнения <
        else:
            # Если значение в колонке меньше заданного значения
            if x < value:
                # Добавляем индекс строки в множество
                s.add(i)
...
if count > 0:
    # Для каждой строки
    for i in range(N):
        # Если индекс строки НЕ в пересечении множеств
        if i not in intersection:
            # Пропускаем неподходящие строки
            continue
